  ...
  async countByMatch(matchId: string): Promise<{ total: number; confirmed: number; live: number }> {
    const result = await getDb().query(
      `SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE is_confirmed=1)::int AS confirmed,
              COUNT(*) FILTER (WHERE is_live=1)::int AS live
         FROM match_player_events WHERE match_id=$1`, [matchId]);
    return result.rows[0] as { total: number; confirmed: number; live: number };
  },

  /** Tras el scrape final, los borradores dejan de ser "en vivo". */
  async clearLiveFlags(matchId: string): Promise<void> {
    await getDb().query('UPDATE match_player_events SET is_live=0 WHERE match_id=$1', [matchId]);
  },
